    ...
    for classes in classes_list:
        ...
        for sig2 in sig2s:
            ...
            for j in range(num_sam):
                denoised_images[j,:] = expectation.depainting2(model, As[j], noisy_images[j], K = 15, sigma = sig2)
            ...
            psnr1 = psnr(denoised_images[:num_sam,:],test_x[:num_sam,:])
            psnr2 = psnr(noisy_images[:num_sam,:],test_x[:num_sam,:])
            ...
            utils.mkdir_p(f'../experiments/inpainting_prop/painted/')
            utils.mkdir_p(f'../experiments/inpainting_prop/depainted/')

            # Save the noisy and denoised images as raw tensors
            torch.save(noisy_images[:num_sam,:], f'../experiments/inpainting_prop/painted/mnist_{i}_sigma_{sig2}.pt')
            torch.save(denoised_images[:num_sam,:], f'../experiments/inpainting_prop/depainted/mnist_{i}_sigma_{sig2}.pt')
