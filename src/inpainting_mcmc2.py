...
inpainted = torch.load('../experiments/inpainting_prop/Figure/inpainted_images.pt')
As = torch.load('../experiments/inpainting_prop/Figure/A_matrix.pt')


for idx,prop in enumerate(props):
    noisy_image = inpainted[idx].reshape(28,28).float()
    print(torch.min(noisy_image), torch.max(noisy_image))
    A = torch.diagflat(As[idx])
    AT = A
    ...
    for classes in classes_list:
        ...
        psnrs.append(f'mnist_{i}, prop = {prop}: {psnr(current_mean.detach(),true_image):.2f}')

    print(psnrs)


utils.mkdir_p('../experiments/inpainting_prop/PC_MCMC')
torch.save(posterior_means,'../experiments/inpainting_prop/PC_MCMC/Final_inpainting_results.pt')
